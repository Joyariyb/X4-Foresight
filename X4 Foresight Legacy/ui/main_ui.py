...
Usage:
    ...
    python ui/main_ui.py --json path/to/x4_empire_state.json
"""
...
def main():
    ...
    parser.add_argument(
        "--json",
        # Default: look for x4_empire_state.json in the project root (one level up)
        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "x4_empire_state.json"),
        help="Path to empire state JSON (default: <project_root>/x4_empire_state.json)",
    )
    args = parser.parse_args()
    data = load_json(args.json)

    # ui.html lives in the same folder as this script (ui/)
    html_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ui.html")
    if not os.path.exists(html_path):
        print("[Error] ui.html not found — make sure it is in the ui/ directory.")
        sys.exit(1)
