...
# Single source of truth for the abundance ladder: the save scanner ranks yield
# levels with this same table, so the static SECTOR_RESOURCES aggregation can
# never disagree with what a real scan of the same sector would produce.
from scanner.handlers.resource import ResourceHandler
...
# Every generated file opens with a file-specific "# Core role:" line plus this
# shared provenance line — the two-line header style data/ was normalised to
# (COMMENT_STYLE.md), so regenerating doesn't churn the committed headers.
GENERATED_BY_LINE = (
    "# Generated by gamefiles/generate_data.py from X4 game files; "
...
def gen_wares_py(wares, group_textids, texts) -> str:
    ...
    parts = [
        "# Core role: Auto-generated ware catalog "
        "(id → {name, volume, transport, group, group_priority}).\n"
        + GENERATED_BY_LINE,
        _fmt_dict(
...
def gen_ware_prices_py(wares) -> str:
    ...
    return ("# Core role: Auto-generated ware price catalog "
            "(ware → {min, average, max, confidence}).\n"
            + GENERATED_BY_LINE + "\n" + _fmt_dict(
...
def gen_production_stats_py(wares) -> str:
    ...
    for w in wares:
        ...
        blocks.append(
            ...
            f"        }},\n"
            f"    }},")
    return ("# Core role: Auto-generated production module catalog "
            "(module → {input, output, duration, workforce}).\n"
            + GENERATED_BY_LINE + "\n" +
...
def gen_station_names_py(wares, texts) -> str:
    ...
    lines = [
        "# Core role: Auto-generated factory catalog "
        "(ware → buildable station name and text ID).",
        GENERATED_BY_LINE,
...
def gen_ships_py(ships) -> str:
    ...
    return ("# Core role: Auto-generated ship catalog "
            "(macro → {class, name, hull, price}).\n"
            + GENERATED_BY_LINE + "\n" +
...
def gen_ship_stats_py(ships) -> str:
    ...
    for s in ships:
        blocks.append(
            ...
            f"    }},")
    return ("# Core role: Auto-generated ship catalog "
            "(macro → {class, hull, hardpoints, price, purchasable}).\n"
            + GENERATED_BY_LINE + "\n" +
...
def gen_station_stats_py(idx: CatalogIndex) -> str:
    ...
    return (
        "# Core role: Auto-generated station module + shield equipment catalog "
        "(macro → {max_hull, max_shield, cargo_capacity, produces, name}).\n"
        + GENERATED_BY_LINE + "\n" +
...
# ──────────────────────────────────────────────────────────────────────────
#  sector_stats.py — SECTOR_SUNLIGHT / SECTOR_NAMES / SECTOR_RESOURCES
# ──────────────────────────────────────────────────────────────────────────
...
# Resource-area refs encode ware + abundance: sphere_<size>_<ware>_<level>_<speed>
# (e.g. "sphere_large_ore_high_slow") — the exact same encoding the save file's
...
def gen_sector_stats_py(idx: CatalogIndex, texts_20004: dict) -> str:
    ...
    than on individual sectors. Sector sunlight and resources are derived from
    the parent cluster for any sector that lacks its own entries.
    """
    # ── Merge mapdefaults ────────────────────────────────────────────────
    ...
    # ── Collect sunlights + resource areas by macro type ─────────────────
    cluster_sun: dict[str, float] = {}   # cluster macro → sunlight
    sector_sun:  dict[str, float] = {}   # sector macro  → sunlight
    cluster_res: dict[str, dict[str, str]] = {}  # cluster macro → {ware: level}
    sector_res:  dict[str, dict[str, str]] = {}  # sector macro  → {ware: level}
    all_sectors: set[str]         = set()  # every sector-like macro seen

    rank = ResourceHandler.YIELD_RANK

    def _collect_resources(ds) -> dict[str, str]:
        # A sector usually has several areas per ware at different abundances;
        # keep the highest level per ware — same aggregation the scanner does
        # with a save's <resourceareas>.
        found: dict[str, str] = {}
        for ra in ds.findall("properties/resourceareas/resourcearea"):
            m_r = _RESOURCEAREA_RE.match(ra.get("ref", ""))
            if not m_r:
                continue
            ware, level = m_r.group(1), m_r.group(2)
            if ware not in found or rank.get(level, -1) > rank.get(found[ware], -1):
                found[ware] = level
        return found

    for ds in defaults_el.findall("dataset"):
        macro = ds.get("macro", "")
        is_cluster = bool(_CLUSTER_CLASSIFY_RE.match(macro))
        is_sector  = bool(_SECTOR_CLASSIFY_RE.match(macro))
        if is_sector:
            all_sectors.add(macro.lower())
        if is_cluster or is_sector:
            res = _collect_resources(ds)
            if res:
                (sector_res if is_sector else cluster_res)[macro.lower()] = res
    ...
    # ── Resolve sunlight + resources for every known sector ───────────────
    # Sectors with direct entries use them; others fall back to their parent
    # cluster (Boron DLC pattern). Sectors with no resolution are omitted —
    # the scanner falls back to 1.0 sunlight / no resources for macros not in
    # the tables.
    result: dict[str, float] = {}
    resources: dict[str, list[tuple[str, str]]] = {}
    for sm in sorted(all_sectors):
        m_p = _SECTOR_PARENT_RE.match(sm)
        parent = m_p.group(1) + "_macro" if m_p else ""
        if sm in sector_sun:
            result[sm] = sector_sun[sm]
        elif parent in cluster_sun:
            result[sm] = cluster_sun[parent]
        res = sector_res.get(sm) or cluster_res.get(parent)
        if res:
            # Richest first, alphabetical within a level, so the generated
            # file (and any UI list fed from it) reads best-to-worst.
            resources[sm] = sorted(res.items(),
                                   key=lambda kv: (-rank.get(kv[1], -1), kv[0]))

    # ── Format output ────────────────────────────────────────────────────
    key_width = max((len(k) for k in all_sectors), default=30) + 2

    def _key(sm: str) -> str:
        return ('    "' + sm + '":').ljust(key_width + 5)
    ...
    for sm, sun in sorted(result.items()):
        ...
        lines.append(f"{_key(sm)} {repr(sun)},{comment}")
    lines.append("}")

    names = {sm: _sector_display_name(sm, texts_20004) for sm in sorted(all_sectors)}
    lines += ["", "SECTOR_NAMES: dict[str, str] = {"]
    for sm, name in names.items():
        if name:
            lines.append(f"{_key(sm)} {repr(name)},")
    lines.append("}")

    lines += ["", "SECTOR_RESOURCES: dict[str, list[tuple[str, str]]] = {"]
    for sm, res in sorted(resources.items()):
        name    = names.get(sm, "")
        comment = f"  # {name}" if name else ""
        pairs   = ", ".join(f"({repr(w)}, {repr(lv)})" for w, lv in res)
