...
Domains live in their own files (economy.py, logistics.py, ...); shared
helpers (the _finding() renderer, cross-domain lookups) live in advisors.py.
Adding a new domain (e.g. military) means adding a new file here and one more
`findings += ...` line below — the other domains are untouched.
"""
from __future__ import annotations
import sqlite3

from .advisors import ware_avg_prices, npc_demand_by_ware
from . import economy, logistics
...
def compute_advisors(conn: sqlite3.Connection, scan_id: int | None = None,
    ...
    missing dict (no resolvable current sector) simply means the two
    NPC-facing rules find nothing to report — an empty findings list is a
    completely valid result on a fresh empire.
