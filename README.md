...
Every ship in the fleet will always have a display name. Ships found in `data/ships.py` use their exact in-game name. Ships not in the lookup table (e.g. newly added by a game update before `data/ships.py` has been regenerated) fall back to a name constructed from the macro string itself — faction, size, role, and variant letter (e.g. "Argon L Freighter (B)").
