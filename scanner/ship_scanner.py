...
def resolve_ship_type(macro: str) -> str | None:
    """
    Looks up the ship's type name from the SHIP_NAMES dict in data/ship_scanner.py.

    HOW THIS WORKS:
    SHIP_NAMES maps macro strings like "ship_tel_s_trans_container_01_b_macro"
    to display names like "Magpie Sentinel". This dict was generated by
    generate_ship_names.py, which read the game's language file and macro
    XML files to produce accurate in-game names.

    RETURN VALUE:
    Returns the display name string if found, or None if the macro isn't in
    the lookup (e.g. for very new ships or DLC ships not yet extracted).
    The caller decides what to do with a None result — currently the entry
    dict uses None and display.py falls back to the hull origin label.
    """
    return SHIP_NAMES.get(macro)
            ...
            if inside_ship:
                if event == 'start':
                    ...
                    if ship_depth == 0:
                        ...
                        # Priority order for the ship's display name:
                        #   1. Player-given custom name (set in-game via the
                        #      ship's name field) — stored as the 'name'
                        #      attribute on the component element.
                        #   2. Ship type name from SHIP_NAMES lookup table
                        #      (e.g. "Magpie Sentinel") — derived from the
                        #      macro string via data/ship_scanner.py.
                        #   3. None — display.py will fall back to the code
                        #      (e.g. "LSS-543") as the display identifier.
                        #
                        # WHY CHECK LANG_STRING_RE:
                        # Some 'name' attributes in the save file contain
                        # language reference strings like "{20101,22603}"
                        # rather than actual text. These are placeholders the
                        # game resolves at runtime — we can't display them
                        # so we skip them and fall through to the type lookup.
                        raw_name = se.get('name')
                        if raw_name and not LANG_STRING_RE.match(raw_name):
                            # Priority 1: player-given custom name
                            name = raw_name
                        else:
                            # Priority 2: look up the ship type from macro
                            name = resolve_ship_type(macro)
                            # Priority 3 (implicit): name stays None if not found
