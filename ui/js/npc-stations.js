  ...
  function renderNpcStationsTable() {
    ...
    const rows = allNpcTradePartners.filter(s =>
    ...
    document.getElementById('npc-range-count').textContent =
      `${rows.length} of ${allNpcTradePartners.length} stations match these filters`;
    ...
    document.getElementById('npc-stations-rows').innerHTML = rows.map(s => `
      <tr>
    ...
    document.getElementById('npc-stations-panel').style.display = rows.length ? '' : 'none';
    document.getElementById('npc-stations-empty').style.display = rows.length ? 'none' : 'flex';
