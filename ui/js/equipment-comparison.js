  ...
  // <select> grouped by faction, alphabetical within each group — restricted to
  // the category currently open (reslibCat) so you only ever compare like with
  // like. Same single-pick native select the Hull Comparison picker uses.
  function reslibEquipCompareSelect(which, currentMacro) {
    ...
    for (const [macro, e] of Object.entries(EQUIPMENT_CATALOG)) {
      ...
      byFaction.get(fac).push({ macro, name: (e.name || macro) + (e.mk ? ` Mk${e.mk}` : '') });
    }
    ...
    return `<div class="reslib-fbox">
      <select onchange="reslibSetCompareEquip('${which}', this.value)">
        <option value="" ${currentMacro ? '' : 'selected'}>Select a ${singular}…</option>
        ${groups}
      </select>
    </div>`;
  }
  ...
  // Stats where a *smaller* number wins (e.g. a shield's recharge delay). Every
  // other stat follows the hull-comparison default of "bigger is greener".
  const ECMP_LOWER_BETTER = new Set(['recharge_delay']);
