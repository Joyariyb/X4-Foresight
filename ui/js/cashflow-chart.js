  ...
  // The dispatcher still calls these by name — they are file-global here.
    ...
    // Shared helpers for the cashflow tooltip builders below. cfRow is the
    // no-null-check row variant used by the By-Trade and By-Ware tips; cfFmtU
    // is the shared unit formatter. (fmtC differs by builder, so it stays local.)
  ...
  // The cashflow charts own several hovers. cashflowTip is a simple payload
  // render; the other four locate the nearest data point under the cursor and
  // drive an SVG highlight marker, so their geometry lives here next to the chart
  // data (cashflowDetailData / wareChartData / shipChartData) rather than in the
  // dispatch engine. See tip-registry.js for the handler contract.
